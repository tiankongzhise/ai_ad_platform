<teammate-message teammate_id="general-purpose-4" summary="Complete frontend code review report for all 27 files">
...
- ✅ **P1-1 OAuth跳转**: Token 刷新使用 `window.location.href = '@command:login'` 整页跳转而非弹窗。
...
- **P3 代码质量 (OnboardingWizard/index.tsx:256)**: Step 3 使用 `window.location.href = '@command:crm'` 而非 React Router 的 `navigate('@command:crm')`，会导致整个应用重新加载。
...
- **P2 逻辑隐患 (App.tsx:63-73)**: `PublicRoute` 中已认证用户跳转逻辑：先调用 `fetchStatus()`（异步），然后根据结果决定跳转到 `/dashboard` 或 `/onboarding`。但这个逻辑和后面的 `return <Navigate to="@command:dashboard" replace />`（第88行）矛盾。第88行的 Navigate 总是跳转到 `/dashboard`，而 fetchStatus 的回调可能跳转到 `/onboarding`。由于 fetchStatus 是异步的，第88行会先执行跳转到 dashboard，然后 fetchStatus 完成后又尝试跳转，导致路由闪烁。
